...
blanks<-read_csv("AIRLIFT_mass_repeatability.csv")%>%
dplyr::select(1:3)%>%
rename(date=1, id=2, mass_mg=3) %>%
mutate(
    date = lubridate::dmy(date), 
    id = as.factor(id),
    mass_mg = mass_mg * 1000)
...
#2a
total_na_count <- sum(is.na(blanks)) 
print(total_na_count)

#2b
unique_ids <- unique(blanks$ids)
count_unique_ids <- length(unique_ids)
...
blanks_summary <- blanks %>%
  ...
  summarise(
    ...
    median_mass_mg = median(mass_mg, na.rm = TRUE), 
    sd_mass_mg = sd(mass_mg, na.rm = TRUE)
  )

    print(blanks_summary)
